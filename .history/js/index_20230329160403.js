...
function swap(viTri1, viTri2, outputB6) {
    ...
    if (viTri1 >= daySo.length || viTri2 >= daySo.length) {
        document.querySelector(outputB6).innerHTML = `Vui lòng chỉ nhập vị trí cần đổi trong giới hạn của mảng`;
    }
    for (var i = 0; i < daySo.length; i++) {
        if (i === viTri1) {
            daySo[i] = b;
...
document.querySelector('#btnSwap').onclick = function () {
    swap(+document.querySelector('#vi-tri-1').value, +document.querySelector('#vi-tri-2').value, "#ket-qua-b6")
}
// Bài 7
function sapXep() {
    for (var i = 1; i < daySo.length; i++) {
