...
// Bài 6
function swap(viTri1, viTri2, outputB6) {
  var a = daySo[viTri1];
  var b = daySo[viTri2];
  if (viTri1 >= daySo.length || viTri2 >= daySo.length) {
    document.querySelector(
      outputB6
    ).innerHTML = `Vui lòng chỉ nhập vị trí cần đổi trong giới hạn của mảng`;
  } else {
    for (var i = 0; i < daySo.length; i++) {
      if (i === viTri1) {
        daySo[i] = b;
    ...
    document.querySelector(outputB6).innerHTML = daySo;
  }
}
document.querySelector("#btnSwap").onclick = function () {
  swap(
    +document.querySelector("#vi-tri-1").value,
    +document.querySelector("#vi-tri-2").value,
    "#ket-qua-b6"
  );
};
// Bài 7
...
document.querySelector("#btnSoSanh").onclick = function () {
  if (demSoDuong(daySo) > demSoDuong(daySoThuc)) {
    document.querySelector(
      "#ket-qua-b10-1"
    ).innerHTML = `Dãy số 1 có nhiều số dương hơn `;
  } else if (demSoDuong(daySo) === demSoDuong(daySoThuc)) {
    document.querySelector(
      "#ket-qua-b10-1"
    ).innerHTML = `2 dãy số có cùng số lượng số dương `;
  } else {
    document.querySelector(
      "#ket-qua-b10-1"
    ).innerHTML = `Dãy số 2 sau có nhiều số dương hơn `;
  }
  if (demSoAm(daySo) > demSoAm(daySoThuc)) {
    document.querySelector(
      "#ket-qua-b10-2"
    ).innerHTML = `Dãy số 1 có nhiều số âm hơn `;
  } else if (demSoAm(daySo) === demSoAm(daySoThuc)) {
    document.querySelector(
      "#ket-qua-b10-2"
    ).innerHTML = `2 dãy số có cùng số lượng số âm `;
  } else {
    document.querySelector(
      "#ket-qua-b10-2"
    ).innerHTML = `Dãy số 2 sau có nhiều số âm hơn `;
  }
};
